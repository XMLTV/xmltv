...
sub init
{
	...
	$self->loadfile("$HOME/.xmltv/channels.xml");
	$self->loadfile("./channels.xml");
}
...
{
   	my $self	= shift;
	my $name	= shift || croak("getuniqename", "missing display name to translate");
	my $lang	= shift || $self->{'LANG'};
	...
	foreach my $id (keys %{$self->{'CHANNELS'}})
	{
		...
		foreach my $channelname (@channels)
		{
			...
			{
				if ( (@{$channelname->{'content'}})[0] eq "$name")
				{
					return $id;
				}
			}
...
{
	my $self    = shift;
	my $file 	= shift || croak("loadfile", "missing file name of xml to load");
